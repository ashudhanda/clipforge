...
REM then wraps dist\ClipForge2 into a Setup.exe via Inno Setup.
setlocal
cd /d "%~dp0\.."
...
pyinstaller packaging\clipforge2.spec || exit /b 1

where iscc >nul 2>nul
if %errorlevel%==0 (
    iscc packaging\installer.iss
    echo.
    echo DONE: dist\ClipForge2-Setup-*.exe
) else (
    echo Inno Setup not found - install it from https://jrsoftware.org/isinfo.php
    echo Portable build ready at: dist\ClipForge2\ClipForge2.exe
)
pause
